...
function normalizeProperties(properties, name) {
  ...
  } else if(properties.type.name && properties.type.name !== 'SchemaObjectInstance') {
...
class SchemaObject {
  constructor(schema, options) {
    return SchemaObjectInstanceFactory(schema, options);
  }
}

// Represents an object INSTANCE with typed indexes.
function SchemaObjectInstanceFactory(schema, options) {
  ...
  class SchemaObjectInstance {
    constructor(defaults) {
      ...
      // Normalize schema properties to allow for letious shorthand declarations.
