...
var SchemaObject = function(schema, options) {
  ...
  return function(defaults) {
      ...
      if(properties.isAlias !== true) {
        if(properties.type === 'object') {
          if (properties.default) {
            obj[objIndex] = _.isFunction(properties.default) ? properties.default.call(this) : properties.default;
          } else {
    ...
    // Without Proxy we must register individual getter/setters to put any logic in place.
    // With Proxy, we still use the individual getter/setters, but also catch values that aren't in the schema.
    if(typeof(Proxy) !== 'undefined') {
      proxy = Proxy(self, {
        get: function(target, name, receiver) {
          ...
          // Typically, the registered getter will find and return the value.
          return self[name];
        },
        set: function(target, name, value, receiver) {
          if(options.dotNotation && name.indexOf('.') !== -1) {
            ...
            // Ensure the first-level field exists in the schema.
            // If it does not, a generic Object will suffice.
            // This is necessary for toObject to see the field.
            if(!schema[fields[0]]) {
              schema[fields[0]] = normalizeProperties({ type: 'object' }, fields[0]);
            }
            ...
            for (var i = 0, n = fields.length; i < n && result !== undefined; i++) {
              ...
              if(i === (n - 1)) {
                ...
                if(!_.isObject(result[field])) {
                  // TODO: We should throw a SetterError here, but this part of the setter isn't wired to be able to throw errors.
                  return;
                }
