...
function clearField(index, properties) {
  ...
  // Other field types can simply have their value set to the default (which might be undefined)
    ...
    writeValue.call(this[_privateKey]._this, properties.default, properties);
  }
};
  ...
  class SchemaObjectInstance {
    constructor(defaults) {
      ...
      if(typeof(Proxy) !== 'undefined') {
        const proxy = this[_privateKey]._this = new Proxy(this, {
          ...
          enumerate: (target) => {
            return _.keys(schema)[Symbol.iterator]();
          },
          ...
          get: (target, name, receiver) => {
            if(options.dotNotation && name.indexOf('.') !== -1) {
              ...
              let result = this[_privateKey]._obj;
              let field;
          ...
          set: (target, name, value, receiver) => {
            if(options.dotNotation && name.indexOf('.') !== -1) {
              // Dot notation is on and the field name contains a dot.
              const fields = name.split('.');
              let result = this[_privateKey]._this;
              let field;
              for (let i = 0, n = fields.length; i < n && result !== undefined; i++) {
                field = fields[i];
    ...
    toObject() {
      ...
      _.each(this[_privateKey]._schema, (properties, index) => {
        ...
        const value = this[_privateKey]._this[index];
        ...
        if(value === undefined && this[_privateKey]._options.setUndefined !== true) {
          return;
        }
      ...
      if(_.isFunction(this[_privateKey]._options.toObject)) {
        getObj = this[_privateKey]._options.toObject.call(this, getObj);
      }
