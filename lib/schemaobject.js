...
if(typeof(Proxy) !== 'undefined') {
  ...
  console.warn('You are running node without the --harmony and --harmony_proxies flags. SchemaObject will work, but options strict: false and dotNotation: true will fail.')
}
...
function normalizeProperties(properties, name) {
  ...
  } else if(_.isString(properties.type.name) && properties.type.name !== 'SchemaObjectInstance' && _.keys(properties.type).length === 0) {
...
function defineGetter(index, properties) {
  ...
  this.__defineGetter__(index, () => {
    try {
      return getter.call(this, this._obj[indexOrAliasIndex], properties);
    } catch(error) {
...
function defineSetter(index, properties) {
  this.__defineSetter__(index, (value) => {
    ...
    try {
      // this._this[index] is used instead of this._obj[index] to route through the public interface.
      writeValue.call(this._this, typecast.call(this, value, this._this[index], properties), properties);
    } catch(error) {
...
class SchemaArray extends Array {
  constructor(self, properties) {
    ...
    if(this._properties.arrayType) {
      this._properties.arrayType = normalizeProperties.call(this._self, this._properties.arrayType);
    }
  }
  ...
  class SchemaObjectInstance {
    constructor(defaults) {
      ...
      _.each(schema, (properties, index) => {
        ...
        defineGetter.call(this, index, properties);
        defineSetter.call(this, index, properties);
      });
    ...
    // Clear all values.
    clear() {
      _.each(this._schema, (properties, index) => {
        clearField.call(this._this, index, properties);
      });
    }
