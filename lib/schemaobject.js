...
class SchemaArray extends Array {
  ...
  concat() {
    ...
    for(let argArray of arguments) {
      if(argArray.toArray) {
        argArray = argArray.toArray();
      }
      array = array.concat(argArray);
    }

    // Return new instance of SchemaArray.
    const schemaArray = new SchemaArray(this[_privateKey]._self, this[_privateKey]._properties);
    SchemaArray.prototype.push.apply(schemaArray, array);
...
function SchemaObjectInstanceFactory(schema, options) {
  ...
  if(typeof(Proxy) === 'undefined') {
    ...
    // If keys ignore case is on but the --harmony flag is not, fail.
    if(options.keysIgnoreCase) {
      throw new Error('Keys ignore case support requires --harmony flag.');
    }
  }
